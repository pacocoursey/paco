...
When I started learning Node.js and using ZEIT's Now, the simplicity of sending an email disappeared. Until recently, when it finally clicked.
...
<CLI>touch now.json .nowignore</CLI>
...
Let's only accept `POST` requests and make sure that the body of the requests contains a `name`. If these conditions are not met we return an error.
...
We also want to make sure that our API request gets routed correctly. We specify the routes in `now.json` to redirect `/api/contact` to our Node.js endpoint:
